...
public class HitLog
{
    ...
    public static HashSet<HitLogEntry> entries = new HashSet<HitLogEntry>();
    private static Dictionary<int, List<HitLogEntry>> entriesByPlayerId = new Dictionary<int, List<HitLogEntry>>();
    ...
    public static void AddEntry(ClientInfo source, ClientInfo attacker, ClientInfo victim, EntityPlayer playerA, EntityPlayer playerV, int damage, int armorDamage, bool fatal, ItemValue weapon, Utils.EnumHitDirection direction, EnumBodyPartHit hitbox, float fps)
    {
        ...
        lock (entriesByPlayerIdLock)
        {
            if (!entriesByPlayerId.ContainsKey(attacker.entityId))
                entriesByPlayerId[attacker.entityId] = new List<HitLogEntry>();
            entriesByPlayerId[attacker.entityId].Add(entry);

            if (!entriesByPlayerId.ContainsKey(victim.entityId))
                entriesByPlayerId[victim.entityId] = new List<HitLogEntry>();
    ...
    {
        return entriesByPlayerId.TryGetValue(playerId, out var playerEntries) ? playerEntries : new List<HitLogEntry>();
    }
    public static List<HitLogEntry> GetEntriesForPlayer(int playerId, DateTime? startTime = null, DateTime? endTime = null)
    {
    ...
    public static List<HitLogEntry> GetFatalOnly(List<HitLogEntry> list = null)
    {
        if (list != null)
            return list.Where(entry => entry.fatal).ToList();
        return entries.Where(entry => entry.fatal).ToList();
    }
    public static List<HitLogEntry> GetHitsOnly(List<HitLogEntry> list = null)
    {
        return GetNonFatalOnly(list);
    }
    public static List<HitLogEntry> GetNonFatalOnly(List<HitLogEntry> list = null)
    {
        if (list != null)
            return list.Where(entry => !entry.fatal).ToList();
        return entries.Where(entry => !entry.fatal).ToList();
    ...
    // All melee hits
    public static List<HitLogEntry> GetMeleeOnly(List<HitLogEntry> list = null)
    {
        if (list != null)
    ...
    // All gun hits
    public static List<HitLogEntry> GetGunOnly(List<HitLogEntry> list = null)
    {
        if (list != null)
    ...
    {
        List<HitLogEntry> query = (List<HitLogEntry>)entries.Where(entry => entry.fatal);
        if (startTime.HasValue)
        {
            query = (List<HitLogEntry>)query.Where(entry => entry.timestamp >= startTime.Value);
        }
        if (endTime.HasValue)
        {
            query = (List<HitLogEntry>)query.Where(entry => entry.timestamp <= endTime.Value);
        }
    ...
    {
        List<HitLogEntry> query = entries.ToList();
        if (startTime.HasValue)
        {
            query = (List<HitLogEntry>)query.Where(entry => entry.timestamp >= startTime.Value);
        }
    ...
    public static List<int> GetPlayerRankingByCriteria(Func<List<HitLogEntry>, int> rankingCriteria, DateTime? startTime = null, DateTime? endTime = null)
    {
