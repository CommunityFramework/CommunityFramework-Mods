...
public class HitLogEntry
{
    ...
    public HitLogEntry(ClientInfo source, ClientInfo attacker, ClientInfo victim, EntityPlayer playerA, EntityPlayer playerV, int damage, int armorDamage, bool fatal, ItemValue weapon, Utils.EnumHitDirection direction, EnumBodyPartHit hitbox, float fps)
    {
    ...
    public string ToStringAll()
    {
        return $"{timestamp}: " +
               ...
               $"AttPos: {(int)attackerPos.x} {(int)attackerPos.y} {(int)attackerPos.z} " +
               $"VicPos: {(int)victimPos.x} {(int)victimPos.y} {(int)victimPos.z} " +
               $"FPS: {fps:F1} " +
